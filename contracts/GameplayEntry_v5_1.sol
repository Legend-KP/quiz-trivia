...
/**
 * @title GameplayEntry  Final Secure Version v5.1
 * @dev Collects 0.1 USDT per gameplay on Celo Mainnet
 * @notice No constructor arguments needed  just deploy and go
 *
 * Security fixes applied from audit:
 * - Reentrancy guard (uint256 pattern  safe on revert)
 * - Explicit allowance + balance checks before transfer
 * - Rate limiting (10 seconds between payments per wallet)
 * - newOwner cannot be USDT address or zero address
 * - getBalance() failure handled with revert not silent 0
 ...
 *
 * v5.1 changes:
 * - FEE = 0.1 USDT (100_000 raw units, 6 decimals)
 * - totalWithdrawn tracker added for accounting clarity
 * - getStats() view function  returns lifetimeCollected + currentBalance
 */
...
contract GameplayEntry {
    ...
    /**
     * @dev Pay 0.1 USDT to submit a gameplay entry
     *
     * Step 1: On USDT contract call approve(THIS_CONTRACT_ADDRESS, 100000)
     * Step 2: Call pay() here  0.1 USDT moves to this contract
     *
     * Protected by:
    ...
    function pay() external nonReentrant whenNotPaused {
        ...
        require(
            allowed >= FEE,
            "Insufficient USDT allowance. Call approve(contractAddress, 100000) on USDT first"
        );
        ...
        require(
            bal >= FEE,
            "Insufficient USDT balance. You need at least 0.1 USDT"
        );
    ...
    /**
     * @dev Returns lifetime inflows and current withdrawable balance separately.
     * lifetimeCollected = all USDT ever received via pay()
     * currentBalance    = USDT sitting in contract right now
     *                   = totalCollected - totalWithdrawn
     */
    function getStats() external view returns (
        uint256 lifetimeCollected,
        uint256 currentBalance
    ) {
        return (totalCollected, getBalance());
    }
